...
To retrieve a service costs summary for the specified customer, first call the [**IAggregatePartner.Customers.ById**](pc_sdk_cust.icustomercollection_byid) method with the customer ID to identify the customer. Then use the [**ServiceCosts**](pc_sdk_cust.icustomer_servicecosts) property to get an interface to customer service costs collection operations. Next, call the [**ByBillingPeriod**](pc_sdk_cust_servicecosts.icustomerservicecostscollection_bybillingperiod) method with a member of the billing period enumerator to return an [**IServiceCostsCollection**](pc_sdk_cust_servicecosts.iservicecostscollection). Finally, use the [**IServiceCostsCollection.LineItems.Get**](pc_sdk_cust_servicecosts.iservicecostlineitemscollection_get) or [**GetAsync**](pc_sdk_cust_servicecosts.iservicecostlineitemscollection_getasync) method to get the customer’s service costs line items.
...
If successful, the response body contains a [ServiceCostLineItem](pc_apiv2.servicecostssummary) resource that provides information about the service costs.
