...
To get a collection of all available invoices, use the [**Invoices**](pc_sdk.ipartner_invoices) property to get an interface to invoice operations, and then call the [**Get**](pc_sdk_invoices.iinvoicecollection_get) or [**GetAsync**](pc_sdk_invoices.iinvoicecollection_getasync) method to retrieve the collection.

To get a paged collection of invoices, first call the [**BuildIndexedQuery**](pc_sdk_models_query.queryfactory_buildindexedquery) method and pass it the page size to create an [**IQuery**](pc_sdk_models_query.iquery) object. Next, use the [**Invoices**](pc_sdk.ipartner_invoices) property to get an interface to invoice operations, and then pass the IQuery object to the [**Query**](pc_sdk_invoices.iinvoicecollection_query) or [**QueryAsync**](pc_sdk_invoices.iinvoicecollection_queryasync) method to send the request and get the first page.

Next, use the [**Enumerators**](pc_sdk.ipartner_enumerators) property to get an interface to the collection of supported resource collection enumerators, and then call [**Invoices.Create**](pc_sdk_factory.iresourcecollectionenumeratorfactory_create) to create an enumerator for traversing the collection of invoices. Finally, use the enumerator to retrieve and work with each page of invoices as shown in the following code example. Each call to the [**Next**](pc_sdk_enumerators.iresourcecollectionenumerator_next) method sends a request for the next page of invoices based on the page size.
