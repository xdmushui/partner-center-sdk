...
To update a customer's service request, call the [**IServiceRequestCollection.ById**](pc_sdk_servicereq.iservicerequestcollection_byid) method with the service request id to identify and return the service request interface. Then call the [**IServiceRequest.Patch**](pc_sdk_servicereq.iservicerequest_patch) or [**PatchAsync**](pc_sdk_servicereq.iservicerequest_patchasync) method to update the service request. To provide the updated values, create a new, empty [**ServiceRequest**](pc_sdk_models_servicreq.servicerequest) object and set only the property values that you want to change. Then pass that object in the call to the Patch or PatchAsync method.
