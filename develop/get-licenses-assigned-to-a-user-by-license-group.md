...
To check which licenses are assigned to a user from specified license groups, start by instantiating a [List](https://msdn.microsoft.com/en-us/library/6sh2ey19.aspx) of type [**LicenseGroupId**](pc_sdk_models_lic.licensegroupid), and then add the license groups to the list. Then, use the [**IAggregatePartner.Customers.ById**](pc_sdk_cust.icustomercollection_byid) method with the customer ID to identify the customer. Next, call the [**Users.ById**](pc_sdk_cust_user.icustomerusercollection_byid) method with the user ID to identify the user. Then, get an interface to customer user license operations from the [**Licenses**](pc_sdk_cust_user.icustomeruser_licenses) property. Finally, pass the list of license groups to the [**Get**](pc_sdk_cust_user.icustomeruserlicensecollection_get) or [**GetAsync**](pc_sdk_cust_user.icustomeruserlicensecollection_getasync) method to retrieve the collection of licenses assigned to the user.
