...
<tbody>
	...
	<tr>
		...
		<td>string</td>
		<td>
			<p>The <em>filter</em> parameter of the request contains one or more statements that filter the rows in the response. Each statement contains a field and value that are associated with the <strong>eq</strong> or <strong>ne</strong> operators, and statements can be combined using <code>and</code> or <code>or</code>. You can specify the following fields:</p>
			<ul>
	...
	<tr>
		...
		<p><em>aggregationLevel</em> is not supported without a <code>groupby</code>. <em>aggregationLevel</em> applies to all <code>datefields</code> present in the <code>groupby</code></p>
		</td>
	</tr>
	<tr>
		<td>orderby</td>
	...
	<tr>
		...
		<td>
			...
			<p>The returned data rows will contain the fields specified in the <code>groupby</code> parameter as well as the following fields:</p>
			<ul>
				<li><em>indirectResellerCount</em></li>
				<li><em>licenseCount</em></li>
				<li><em>subscriptionCount</em></li>
			</ul>
			<p>The <code>groupby</code> parameter can be used with the <em>aggregationLevel</em> parameter.</p>
			<p><strong>Example:</strong></br>
